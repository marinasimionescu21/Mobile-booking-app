...
class Chart extends StatelessWidget {
  ...
  double get maxTotalHotels {
    double maxTotalHotels = 0;

    for (final bucket in buckets) {
      if (bucket.totalPrice > maxTotalHotels) {
        maxTotalHotels = bucket.totalPrice;
      }
    }

    return maxTotalHotels;
  }
                ...
                for (final bucket in buckets) // alternative to map()
                  ChartBar(
                    fill: bucket.totalPrice == 0
                        ? 0
                        : bucket.totalPrice / maxTotalHotels,
                  )
              ],
            ),
          ),
